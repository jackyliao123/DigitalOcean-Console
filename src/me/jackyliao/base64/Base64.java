...
public class Base64 {
	...
	public static String encode(byte[] array, int offset, int length) {
		StringBuilder builder = new StringBuilder((length * 4 + 2) / 3);
		...
		for(int i = 0; i < length; i += 3) {
			...
			c1 |= (array[i + offset] & 0xFC) >>> 2;
			c2 |= (array[i + offset] & 0x3) << 4;

			if(i + 1 < length) {
				c2 |= (array[i + 1 + offset] & 0xF0) >>> 4;
				c3 = (array[i + 1 + offset] & 0x0F) << 2;
			}
			if(i + 2 < length) {
				c3 |= (array[i + 2 + offset] & 0xC0) >>> 6;
				c4 = (array[i + 2 + offset] & 0x3F);
			}
